...
import CategoriesSidebar from "./list";
import CategoriesMain from "./details";
import Slider from "./slider";
import LoadSpinner from "../../load/load";
import { Sparkles, TrendingUp, RefreshCw, Info, ArrowLeft } from "lucide-react";
import { Hero } from "./hero";

export default function ClientHome({ initialChances = [], fetchError = false }) {
  const [chances, setChances] = useState(initialChances);
  const [selectedChance, setSelectedChance] = useState(null);
  const [loading, setLoading] = useState(false);
  const [error, setError] = useState(fetchError ? "حصل خطأ في تحميل الفرص." : null);

  async function fetchChances() {
    setLoading(true);
    setError(null);
    try {
      // بندّي طلب للـ API proxy داخل نفس النكست عشان نتخطى أي CORS
      const res = await fetch("/api/proxy/invests");
                    ...
                    <div className="mx-auto">
                      <div className="flex flex-col md:flex-row items-center justify-between gap-6">
                        <div className="flex items-center gap-4">
                          ...
                          <div>
                            <h2 className="text-3xl md:text-4xl font-bold text-[#1a1a4d]">فرصنا</h2>
