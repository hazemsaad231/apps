...
import ClientHome from "./pages/parts/clinetHome"; 
...
export default async function Home() {
  ...
  if (!res.ok) {
    // ممكن ترجع صفحة خطأ أو تمرر [] للـ client
    return <ClientHome initialChances={[]} fetchError={true} />;
  }

  const data = await res.json();
  const items = data?.data || [];

  return <ClientHome initialChances={items} />;
}
